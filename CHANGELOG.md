...
- **Every filter and sort control now lives inside a real `<form>`,** so filtering works with JavaScript off. This changes markup and store internals a theme or plugin may depend on:
  ...
  - **`data-wp-context` on filter and sort block wrappers no longer carries `queryVar`, `pageVar`, `orderbyVar` or `orderVar`.** `view.js` now reads a control's own `name`, `form` and `value` attributes instead.
